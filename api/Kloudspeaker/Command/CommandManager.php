...
class CommandManager {
	...
	public function execute($name, $commands, $opts) {
		$cmd = $this->cmds[$name];

		if (is_object($cmd)) {
			return $cmd->execute($name, $commands, $opts);
		} else if (is_string($cmd)) {
			return $cmd($name, $commands, $opts);
		} else if (is_callable($cmd)) {
			return call_user_func_array($cmd, [$commands, $opts]);
		} else {
